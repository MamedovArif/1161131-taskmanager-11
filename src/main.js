...
const renderTask = (taskListElement, task) => {
  ...
  const replaceTaskToEdit = () => {
    taskListElement.replaceChild(taskEditElement,
        taskElement);
  };

  const replaceEditToTask = () => {
    taskListElement.replaceChild(taskElement,
        taskEditElement);
  };

  const replaceEditToTaskProvidedEscKeyDown = (evt) => {
    const isEscKey = evt.key === `Escape` || evt.key === `Esc`;

    if (isEscKey) {
      replaceEditToTask();
      document.removeEventListener(`keydown`,
        replaceEditToTaskProvidedEscKeyDown);
    }
  };
...
const renderBoard = (boardComponent, tasks) => {
  ...
  if (isAllTasksArchived) {
    render(boardComponent.getElement(),
        new NoTasksComponent().getElement(), RenderPosition.BEFOREEND);
    return;
  }


  render(boardComponent.getElement(),
      new SortComponent().getElement(), RenderPosition.BEFOREEND);
  render(boardComponent.getElement(),
      new TasksComponent().getElement(), RenderPosition.BEFOREEND);
...
render(siteHeaderElement,
    new SiteMenuComponent().getElement(), RenderPosition.BEFOREEND);
render(siteMainElement,
    new FilterComponent(filters).getElement(), RenderPosition.BEFOREEND);
